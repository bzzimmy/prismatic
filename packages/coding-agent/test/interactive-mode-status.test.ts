...
describe("InteractiveMode.showLoadedResources", () => {
	...
	test("shows a compact resource summary by default", () => {
		...
		expect(output).toContain("1 skill loaded");
	...
	test("summarizes extensions with a count in compact listing", () => {
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
		...
		const output = renderAll(fakeThis.loadedResourcesContainer);
		expect(output).toContain("2 extensions loaded");
	...
	test("captures mixed extension layouts in compact output", () => {
		...
		expect(normalizeRenderedOutput(fakeThis.loadedResourcesContainer)).toMatchInlineSnapshot(`"8 extensions loaded"`);
	});
...
"[Extensions]
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		...
		expect(output).toContain("2 context files loaded");
		expect(output).not.toContain("[Context]");
		expect(output).not.toContain("AGENTS.md");
	});

	test("shows system prompt context paths before project context files when expanded", () => {
		...
		const systemIndex = output.indexOf("SYSTEM.md");
		const appendIndex = output.indexOf("APPEND_SYSTEM.md");
		const agentsIndex = output.indexOf("AGENTS.md");
		expect(systemIndex).toBeGreaterThanOrEqual(0);
		expect(systemIndex).toBeLessThan(appendIndex);
		expect(appendIndex).toBeLessThan(agentsIndex);
	});
